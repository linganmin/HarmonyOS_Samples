...
#ifndef VIDEOPROCESSING_PLUGINMANAGER_H
#define VIDEOPROCESSING_PLUGINMANAGER_H
...
public:
    ...
    OHNativeWindow *pluginWindow_ = nullptr; //docoder ->pluginWindow_
    ...
    uint32_t *processCount = nullptr;
    SampleInfo *sampleInfo_ = nullptr;
    bool isVPRunning = false;
    int32_t frameCount = 0;
    std::unique_ptr<std::thread> OnErrorThread_ = nullptr;
    int32_t errorCode = 0;
    void InitProcessing(SampleInfo &sampleInfo);
    void PrepareSurface(SampleInfo &sampleInfo);
    void StartProcessing();
    void StopProcessing();
    void SetSampleInfo(SampleInfo *sampleInfo);
    void DestroyProcessing();
    void Release();
    void ResetVP(int32_t ret, int8_t from);
    int32_t BlackLastFrame();

private:
    static PluginManager pluginManager_;
    bool isEnvironmentInited = false;
    void VPOnErrorThread();
    std::unordered_map<std::string, OH_NativeXComponent *> nativeXComponentMap_;
    std::unordered_map<std::string, PluginRender *> pluginRenderMap_;
};
}
#endif //VIDEOPROCESSING_PLUGINMANAGER_H
