...
bool PluginRender::m_stop = false;
...
void PluginRender::RenderThread()
{
    while (true) {
        if (!m_stop) {
            uint64_t width;
            uint64_t height;
            int32_t ret = OH_NativeXComponent_GetXComponentSize(m_component, m_window, &width, &height);
            m_width = width;
            m_height = height;
            m_eglCore->Init(m_window, m_width, m_height);
            m_eglCore->SetMode(m_mode, m_useVRS);
            m_eglCore->DrawLoop();
        } else {
...
{
    LOGI("PluginRender OnSurfaceChangedCB");
}
...
napi_value PluginRender::SetUpscaleMethod(napi_env env, napi_callback_info info)
{
    ...
    char idStr[OH_XCOMPONENT_ID_LEN_MAX + 1] = { '\0' };
    uint64_t idSize = OH_XCOMPONENT_ID_LEN_MAX + 1;
    if (OH_NATIVEXCOMPONENT_RESULT_SUCCESS != OH_NativeXComponent_GetXComponentId(nativeXComponent, idStr,
        &idSize)) {
