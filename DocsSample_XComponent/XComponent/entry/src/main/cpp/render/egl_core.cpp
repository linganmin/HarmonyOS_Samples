...
void EGLCore::CreateStar()
{
    starVertices.clear();
    float ratio = 1.0f;
    constexpr float inner = 0.3f;
    constexpr float outer = 0.95f;
    constexpr GLfloat CenterX = 0.0f;
    constexpr GLfloat CenterY = 0.0f;
    constexpr float pentaAngle = 2. * PI / 5.f;
    for (int i = 0; i < 5; ++i) {
        starVertices.push_back(CenterX);
        starVertices.push_back(CenterY);
        starVertices.push_back(CenterX + inner * cosf(-PI * 0.5f - pentaAngle * 2.f - pentaAngle * i) * ratio);
        starVertices.push_back(CenterY + inner * sinf(-PI * 0.5f - pentaAngle * 2.f - pentaAngle * i));
        starVertices.push_back(CenterX + outer * cosf(PI * 0.5f - pentaAngle * i) * ratio);
        starVertices.push_back(CenterY + outer * sinf(PI * 0.5f - pentaAngle * i));
        starVertices.push_back(CenterX + inner * cosf(-PI * 0.5f - pentaAngle * 3.f - pentaAngle * i) * ratio);
        starVertices.push_back(CenterY + inner * sinf(-PI * 0.5f - pentaAngle * 3.f - pentaAngle * i));
    }
}

void EGLCore::CreateStarColor()
{
    starColors.clear();
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        starColors.push_back(50.f / 255);
        starColors.push_back(50.f / 255);
        starColors.push_back(251.f / 255);
        starColors.push_back(126.f / 255);
        starColors.push_back(126.f / 255);
        starColors.push_back(88.0f / 255);
        starColors.push_back(50.f / 255);
        starColors.push_back(50.f / 255);
...
void EGLCore::DrawTriangleFan(GLint position, const GLfloat shapeVertices[], uint32_t length)
{
    if (position > 0) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EGLCore", "DrawTriangleFan: param error");
        return;
    }
...
void EGLCore::DrawStar() {
    ...
    DrawTriangleFan(position, starVertices.data(), starVertices.size());

    glFlush();
    glFinish();
    if (!eglSwapBuffers(eglDisplay_, eglSurface_)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EGLCore", "DrawStar FinishDraw: failed");
    }
}
