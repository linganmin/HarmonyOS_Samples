...
void EglRenderContext::SetupEglExtensions()
{
    ...
    eglDeleteImageFunc_ = reinterpret_cast<PFNEGLDESTROYIMAGEKHRPROC>(eglGetProcAddress("eglDestroyImageKHR"));
}
...
EGLint EglRenderContext::QueryBufferAge(EGLSurface surface) const
{
    ...
    if (ret == EGL_FALSE) {
        ...
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EglRenderContext",
                     "EglRenderContext::QueryBufferAge: Failed to query on EGLSurface %{public}d, error is %{public}s.",
...
void EglRenderContext::SetDamageRegion(EGLSurface surface, EGLint *damages, EGLint size)
{
    ...
    if (ret == EGL_FALSE) {
        ...
        OH_LOG_Print(
            ...
            "EglRenderContext::SetDamageRegion: Failed to SetDamage on EGLSurface %{public}d, error is %{public}s.",
            surfaceId, GetEglErrorString());
    }
}
...
void EglRenderContext::SwapBuffersWithDamage(EGLSurface surface, const EGLint *damages, EGLint size)
{
    ...
    if (eglSwapBuffersWithDamageFunc_ == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_WARN, LOG_PRINT_DOMAIN, "EglRenderContext",
                     "EglRenderContext::SwapBuffersWithDamage: not supported, use eglSwapBuffers");
    ...
    if (ret == EGL_FALSE) {
        ...
        eglQuerySurface(eglDisplay_, surface, EGL_CONFIG_ID, &surfaceId);
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EglRenderContext",
                     "EglRenderContext::SwapBuffersWithDamage: Failed on EGLSurface %{public}d, error is %{public}s.",
                     surfaceId, GetEglErrorString());
    }
}
...
EGLImageKHR EglRenderContext::CreateEGLImage(OHNativeWindowBuffer *nBuffer)
{
    ...
    img = eglCreateImageFunc_(eglDisplay_, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_OHOS, nBuffer, attrs);
    if (img == EGL_NO_IMAGE_KHR) {
        EGLint error = eglGetError();
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EglRenderContext",
                     "eglCreateImageFunc_ failed, error is %{public}d.",
                     error);
