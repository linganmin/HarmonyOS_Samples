...
bool FileOperator::CopyRawFile(const std::string &rawPath, const std::string &targetPath, bool overWrite)
{
    ...
    do {
        ...
        if (srcFileSize != realReadBytes) {
            LOGE("FileOperator read raw file %s failed, file size: %d, real read: %d", rawPath.c_str(), srcFileSize,
                 realReadBytes);
            break;
        }
...
std::string FileOperator::GetFileAbsolutePath(std::string fileName) { return m_sCurrentHapFilesDir + "/" + fileName; }
