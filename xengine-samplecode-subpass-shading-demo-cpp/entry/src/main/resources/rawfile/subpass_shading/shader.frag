...
layout(std140, set = 0, binding = 1) buffer readonly CameraUbo // FIXME: change back to uniform
{
    mat4 view;
    mat4 proj;
    mat4 projview;
...
layout(std140, set = 1, binding = 1) buffer readonly PointLights // FIXME: change back to uniform // readonly buffer PointLights
{
	int light_num;
...
layout(std140, set = 3, binding = 0) uniform MaterialUbo
{
    int has_albedo_map;
    int has_normal_map;
} material;

layout(set = 3, binding = 1) uniform sampler2D albedo_sampler;
layout(set = 3, binding = 2) uniform sampler2D normal_sampler;
