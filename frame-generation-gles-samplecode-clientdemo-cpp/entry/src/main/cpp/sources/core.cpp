...
Core Core::instance_{};
PredictionMode Core::predictionMode_;
...
void Core::OnCreate(NativeResourceManager *nativeResourceManager) noexcept
{
    ...
    InitCommandHandlers();
    switch (predictionMode_) {
        case PredictionMode::EXTRAPOLATION:
            scene_ = std::make_unique<SceneExtrapolation>();
            break;
        case PredictionMode::INTERPOLATION:
            scene_ = std::make_unique<SceneInterpolation>();
            break;
        default:
            break;
    }
...
{
    if (!scene_->OnFrame()) {
        GOLOGE("Core::OnFrame - Frame rendering failed.");
    }
}
...
napi_value Core::OnCreate(napi_env env, napi_callback_info info)
{
    ...
    predictionMode_ = static_cast<PredictionMode>(*predictionMode);
...
{
    if (!component || !window) {
        GOLOGE("Core::OnXComponentSurfaceCreated invalid parameters");
        return;
    }
