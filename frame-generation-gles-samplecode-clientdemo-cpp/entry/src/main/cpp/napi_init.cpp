...
[[nodiscard]] napi_value Invoke(napi_env env, napi_value exports)
{
    ...
    if (!FrameGeneration::Core::CheckNAPIResult(check, env, "Invoke", "Can't get JS side XComponent")) {
        return nullptr;
    }
    ...
    FrameGeneration::Core::GetInstance()->SetEnv(env);
    if (napi_unwrap(env, xComponentJS, reinterpret_cast<void **>(&xComponent)) == napi_ok) {
        // Request from UI.
        return cases[static_cast<size_t>(FrameGeneration::Core::InitUI(xComponent))];
    }

    // Request from entry ability ArkTS script or from page ArkUI script.
    return cases[static_cast<size_t>(FrameGeneration::Core::InitApp(env, exports))];
}

__attribute__((constructor)) void RegisterFrameGenerationCore() noexcept
{
