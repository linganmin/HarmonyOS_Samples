...
void  vkOBJ::StaticModel::ProcessMaterialTextures(const aiScene* scene)
{
    ...
    for (int i = 0; i < (scene->mNumMaterials); i++) {
        ...
        for (auto type : textureTypes) {
            ProcessMaterialTextureCount(material, type, i);
        }
    }
}

void vkOBJ::StaticModel::ProcessMaterialTextureCount(aiMaterial* material, aiTextureType type, int index)
{
    for (int j = 0; j < material->GetTextureCount(type); j++) {
        ...
        if (item != m_texturesMap.end()) {
            m_textures[index].push_back(item->second);
        } else {
            CreateTexture(path.C_Str(), type, index);
        }
    }
}

void vkOBJ::StaticModel::CreateTexture(const std::string& path, aiTextureType type, int count)
{
    ...
    m_textures[count].push_back(texture);
...
VkImageMemoryBarrier vkOBJ::StaticModel::CreateImageMemoryBarrier(
    std::shared_ptr<Texture> texture, VkImageLayout oldLayout, VkImageLayout newLayout, VkAccessFlags srcAccessMask,
    VkAccessFlags dstAccessMask, VkImageSubresourceRange subresourceRange)
{
...
void vkOBJ::StaticModel::LoadTextureDataToStagingBuffer(std::shared_ptr<vkOBJ::Texture> texture,
    VkMemoryRequirements &memReqs, VkBuffer& stagingBuffer, VkDeviceMemory& stagingMemory, VkFormat& format)
{
    ...
    format = nrComponents == 4 ? VK_FORMAT_R8G8B8A8_UNORM : VK_FORMAT_R8G8B8_UNORM;
    texture->width = width;
    ...
    std::copy(buffer, buffer + bufferSize, data);
    vkUnmapMemory(m_device->logicalDevice, stagingMemory);
    stbi_image_free(buffer);
}

void vkOBJ::StaticModel::CreateImage(VkFormat format, VkMemoryRequirements &memReqs,
...
VkImageBlit vkOBJ::StaticModel::CreateVkImageBlit(std::shared_ptr<Texture> texture, int count)
{
    VkImageBlit imageBlit{};

    imageBlit.srcOffsets[0] = {0, 0, 0};
    imageBlit.srcOffsets[1].x = int32_t(texture->width >> (count - 1));
    imageBlit.srcOffsets[1].y = int32_t(texture->height >> (count - 1));
    imageBlit.srcOffsets[1].z = 1;
    imageBlit.srcSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    imageBlit.srcSubresource.mipLevel = count - 1;
    imageBlit.srcSubresource.baseArrayLayer = 0;
    imageBlit.srcSubresource.layerCount = 1;
    imageBlit.dstOffsets[0] = {0, 0, 0};
    imageBlit.dstOffsets[1].x = int32_t(texture->width >> count);
    imageBlit.dstOffsets[1].y = int32_t(texture->height >> count);
    imageBlit.dstOffsets[1].z = 1;
    imageBlit.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    imageBlit.dstSubresource.layerCount = 1;
    imageBlit.dstSubresource.mipLevel = count;
    imageBlit.dstSubresource.baseArrayLayer = 0;
    return imageBlit;
}

VkBufferImageCopy vkOBJ::StaticModel::CreateBufferImageCopy(std::shared_ptr<Texture> texture)
{
...
void vkOBJ::StaticModel::GenerateMipmaps(VkCommandBuffer blitCmd,
    std::shared_ptr<Texture> texture, VkCommandBuffer copyCmd, VkImageSubresourceRange subresourceRange)
{
    for (uint32_t i = 1; i < texture->mipLevels; i++) {
            VkImageBlit imageBlit = CreateVkImageBlit(texture, i);
...

            {
                VkImageMemoryBarrier imageMemoryBarrier = CreateImageMemoryBarrier(
                    texture, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_UNDEFINED,
                    0, VK_ACCESS_TRANSFER_WRITE_BIT, subresourceRange);
            ...
            {
                VkImageMemoryBarrier imageMemoryBarrier = CreateImageMemoryBarrier(
                    texture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                    VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, subresourceRange);
                vkCmdPipelineBarrier(blitCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0,
                                     nullptr, 0, nullptr, 1, &imageMemoryBarrier);
            }
            VkImageMemoryBarrier imageMemoryBarrier = CreateImageMemoryBarrier(
                texture, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_SHADER_READ_BIT, subresourceRange);
            vkCmdPipelineBarrier(copyCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                                 0, 0, nullptr, 0, nullptr, 1, &imageMemoryBarrier);
        }
}

void vkOBJ::StaticModel::CreateImageViewInfo(VkFormat format, std::shared_ptr<Texture> texture)
{
...
void vkOBJ::StaticModel::CreateSamplerCreateInfo(VkSampler sampler, std::shared_ptr<Texture> texture)
{
...
//         VkDeviceSize bufferSize;
        ...
        LoadTextureDataToStagingBuffer(texture, memReqs, stagingBuffer,
            stagingMemory, format);

        CreateImage(format, memReqs, texture);
        VkImageSubresourceRange subresourceRange = {};
        subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        subresourceRange.levelCount = 1;
        subresourceRange.layerCount = 1;
        VkImageMemoryBarrier imageMemoryBarrier = CreateImageMemoryBarrier(
            texture, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
            0, VK_ACCESS_TRANSFER_WRITE_BIT, subresourceRange);
        vkCmdPipelineBarrier(copyCmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr,
            0, nullptr, 1, &imageMemoryBarrier);
        
        VkBufferImageCopy bufferCopyRegion = CreateBufferImageCopy(texture);

        vkCmdCopyBufferToImage(copyCmd, stagingBuffer, texture->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
            &bufferCopyRegion);
        
        imageMemoryBarrier = CreateImageMemoryBarrier(
            texture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
            VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, subresourceRange);
        ...
        GenerateMipmaps(blitCmd, texture,  copyCmd, subresourceRange);
        ...
        CreateImageViewInfo(format, texture);
        VkSampler sampler;
        CreateSamplerCreateInfo(sampler, texture);
    }
}
