...
public class VerifySignatureUtil {
    ...
    public static boolean verifySignature(Jws jws, String filepath) {
        ...
        try (FileInputStream in = new FileInputStream(filepath)) {
			...
			System.out.println("parse cert failed");
        }
        ...
        System.out.println("Only ES256 is supported");
        return false;
    }

    private static boolean verify(Jws jws) {
        // Verify cert chain
        List<String> certs = jws.getHeader().getX5c();
        if (certs.size() != 3) {
            System.out.println("verify cert chain number failed");
            return false;
        }
        ...
        try {
		    ...
		    System.out.println("verify cert chain failed");
            return false;
        }
        ...
        try {
            ...
            System.out.println("verify cert domain failed");
            return false;
        }
        ...
        try {
            ...
            JWTVerifier verifier = JWT.require(Algorithm.ECDSA256((ECPublicKey)pubKey, null)).build();
            verifier.verify(jws.getToken());
            return true;
        } catch (JWTVerificationException e) {
		    System.out.println("verify jws failed");
            return false;
        }
    }
