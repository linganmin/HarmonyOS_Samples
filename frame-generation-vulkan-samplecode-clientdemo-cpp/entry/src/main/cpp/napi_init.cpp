...
[[nodiscard]] napi_value Invoke(napi_env env, napi_value exports)
{
    ...
    if (!Core::CheckNAPIResult(check, env, "Invoke", "Can't get JS side XComponent")) {
        return nullptr;
    }
    ...
    if (napi_unwrap(env, xComponentJS, reinterpret_cast<void **>(&xComponent)) == napi_ok) {
        // Request from UI.
        return cases[static_cast<size_t>(Core::InitUI(xComponent))];
    }
...
__attribute__((constructor)) void RegisterFrameGenerationVulkanCore() noexcept
{
