...
NDKCamera::NDKCamera(char *str, uint32_t focusMode, uint32_t cameraDeviceIndex, double ratioXC)
    : ratioXC_(ratioXC), previewSurfaceId_(str), cameras_(nullptr), focusMode_(focusMode), cameraDeviceIndex_(cameraDeviceIndex),
...
NDKCamera::~NDKCamera() {
    ...
    if (cameraManager_) {
        ...
        if (ret != CAMERA_OK) {
            ...
            OH_LOG_INFO(LOG_APP, "Release OH_Camera_DeleteCameraManager. ok");
        }
...
Camera_ErrorCode NDKCamera::SessionRelease(void) {
    OH_LOG_INFO(LOG_APP, " enter SessionRelease");
    Camera_ErrorCode ret = OH_CaptureSession_Release(captureSession_);
    captureSession_ = nullptr;
    OH_LOG_INFO(LOG_APP, " exit SessionRelease");
    return ret;
}
...
Camera_ErrorCode NDKCamera::IsVideoStabilizationModeSupportedFn(uint32_t mode) {
    ...
    if (ret == CAMERA_OK) {
        OH_LOG_INFO(LOG_APP, "OH_CaptureSession_GetVideoStabilizationMode success. videoMode：%u ", videoMode);
    } else {
...
Camera_ErrorCode NDKCamera::CreateCameraInput(void) {
    OH_LOG_INFO(LOG_APP, "enter CreateCameraInput.");
...
Camera_ErrorCode NDKCamera::CreatePreviewOutput(void) {
    double ratioNow = 0;
    for (int i = 0; i<cameraOutputCapability_->previewProfilesSize - 1; i++){
        double ratio = double(cameraOutputCapability_->previewProfiles[i]->size.width) /
                       cameraOutputCapability_->previewProfiles[i]->size.height;
        if (abs(ratio - ratioXC_) < abs(ratioNow - ratioXC_)) {
...
Camera_ErrorCode NDKCamera::CreatePhotoOutput(char *photoSurfaceId) {
    double ratioNow = 0;
    for (int i = 0; i<cameraOutputCapability_->photoProfilesSize - 1; i++){
        double ratio = double(cameraOutputCapability_->photoProfiles[i]->size.width) /
...
Camera_ErrorCode NDKCamera::CreateVideoOutput(char *videoId) {
    double ratioNow = 0;
    for (int i = 0; i<cameraOutputCapability_->videoProfilesSize - 1; i++){
        double ratio = double(cameraOutputCapability_->videoProfiles[i]->size.width) /
...
Camera_ErrorCode NDKCamera::PreviewOutputStop(void) {
    OH_LOG_INFO(LOG_APP, "enter PreviewOutputStop.");
...
Camera_ErrorCode NDKCamera::PreviewOutputRelease(void) {
    OH_LOG_INFO(LOG_APP, "enter PreviewOutputRelease.");
...
Camera_ErrorCode NDKCamera::PhotoOutputRelease(void) {
    OH_LOG_INFO(LOG_APP, "enter PhotoOutputRelease.");
...
Camera_ErrorCode NDKCamera::IsExposureBiasRange(int exposureBias) {
    ...
    OH_LOG_INFO(LOG_APP, "OH_CaptureSession_SetExposureBias end.");
    if (ret_ != CAMERA_OK) {
...
Camera_ErrorCode NDKCamera::TakePicture(void) {
    ...
    OH_LOG_INFO(LOG_APP, "takePicture OH_PhotoOutput_Capture ret = %{public}d.", ret);
    if (ret != CAMERA_OK) {
        OH_LOG_ERROR(LOG_APP, "startPhoto failed.");
        return CAMERA_INVALID_ARGUMENT;
    }
...
Camera_ErrorCode NDKCamera::TakePictureWithPhotoSettings(Camera_PhotoCaptureSetting photoSetting) {
    ...
    OH_LOG_INFO(LOG_APP, "takePicture TakePictureWithPhotoSettings ret = %{public}d.", ret);
    if (ret != CAMERA_OK) {
        OH_LOG_ERROR(LOG_APP, "startPhoto failed.");
        return CAMERA_INVALID_ARGUMENT;
    }
...
void CameraManagerStatusCallback(Camera_Manager *cameraManager, Camera_StatusInfo *status) {
    if (cameraManager == nullptr || status == nullptr) {
        OH_LOG_ERROR(LOG_APP, "CameraManager or status is nullptr.");
    }
...
void OnCameraInputError(const Camera_Input *cameraInput, Camera_ErrorCode errorCode) {
    if (cameraInput == nullptr) {
        OH_LOG_ERROR(LOG_APP, "CameraInput is nullptr.");
    }
...
void PhotoOutputOnFrameShutter(Camera_PhotoOutput *photoOutput, Camera_FrameShutterInfo *info) {
    if (photoOutput == nullptr || info == nullptr) {
        OH_LOG_ERROR(LOG_APP, "PhotoOutput or info is nullptr.");
    }
...
void VideoOutputOnFrameStart(Camera_VideoOutput *videoOutput) {
    if (videoOutput == nullptr) {
        OH_LOG_ERROR(LOG_APP, "VideoOutput is nullptr.");
    }
