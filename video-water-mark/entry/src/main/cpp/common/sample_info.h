...
struct SampleInfo {
    ...
    std::string videoCodecMime = MIME_VIDEO_AVC.data();
    ...
    OHNativeWindow *nativeImageWindow = nullptr;
...
public:
    ...
    {
        if (bufferLen + remainlen_ > cache_.size()) {
            cache_.resize(remainlen_ + bufferLen);
        }
        std::memcpy(cache_.data() + remainlen_, buffer, bufferLen);
        remainlen_ += bufferLen;
    }

    bool ReadCache(void *buffer, int32_t bufferLen)
    {
        if (remainlen_ < bufferLen) {
            return false;
        }
        std::memcpy(buffer, cache_.data(), bufferLen);
        remainlen_ = remainlen_ - bufferLen;
        if (remainlen_ > 0) {
            std::memmove(cache_.data(), cache_.data() + bufferLen, remainlen_);
        }
        return true;
    }
