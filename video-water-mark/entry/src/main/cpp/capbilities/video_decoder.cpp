...
int32_t VideoDecoder::Config(const SampleInfo &sampleInfo, CodecUserData *codecUserData)
{
    ...
    // SetSurface from video decoder
     if (sampleInfo.nativeImageWindow != nullptr) {
        int ret = OH_VideoDecoder_SetSurface(decoder_, sampleInfo.nativeImageWindow);
        CHECK_AND_RETURN_RET_LOG(ret == AV_ERR_OK && sampleInfo.nativeImageWindow, AVCODEC_SAMPLE_ERR_ERROR,
                                 "Set surface failed, ret: %{public}d", ret);
    } else if (sampleInfo.window != nullptr) {
