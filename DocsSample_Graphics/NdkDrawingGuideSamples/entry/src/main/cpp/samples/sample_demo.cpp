...
napi_value SampleBitMap::NapiDrawPattern(napi_env env, napi_callback_info info)
{
    ...
    int32_t drawOption;
    napi_get_value_int32(env, args[0], &drawOption);                               // 4 means the forth argument
    NativePixelMap *nativePixelMap = OH_PixelMap_InitNativePixelMap(env, args[1]); // 5 means the forth argument
    if (render != nullptr) {
        render->Prepare();
        render->Create();
        render->Draw(static_cast<DrawType>(drawOption), nativePixelMap);
        render->DisPlay();
        render->Destroy();
...
void SampleBitMap::Draw(DrawType type, NativePixelMap* nativePixelMap)
{
    NativeDraw nativeDraw(cCanvas_, width_, height_);
    if (type == CANVASSTATE) {
        nativeDraw.Clip();
        nativeDraw.DrawMatrix();
        nativeDraw.MoveClip();
    } else if (type == SHAPE) {
        nativeDraw.DrawShapeWithPen();
        nativeDraw.DrawShapeWithBrush();
    } else if (type == PATH) {
        nativeDraw.DrawComplexPath();
        nativeDraw.DrawPathBrush();
        nativeDraw.DrawPathLine();
        nativeDraw.DrawPathShade();
    } else if (type == PICTEXT) {
        nativeDraw.DrawImageRect();
        nativeDraw.DrawPixelMapBrush(nativePixelMap);
        nativeDraw.DrawTextBlob();
    } else if (type == COMPLEX) {
        nativeDraw.DrawComplex();
    }
}
