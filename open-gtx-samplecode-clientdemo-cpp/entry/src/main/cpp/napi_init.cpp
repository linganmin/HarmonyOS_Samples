...
[[nodiscard]] napi_value Invoke(napi_env env, napi_value exports)
{
    ...
    if (!OpenGTX::Core::CheckNAPIResult(check, env, "Invoke", "Can't get JS side XComponent")) {
        return nullptr;
    }
    ...
    OpenGTX::Core::GetInstance()->SetEnv(env);
    if (napi_unwrap(env, xComponentJS, reinterpret_cast<void **>(&xComponent)) == napi_ok) {
        // Request from UI.
        return cases[static_cast<size_t>(OpenGTX::Core::InitUI(xComponent))];
    }

    // Request from entry ability ArkTS script or from page ArkUI script.
    return cases[static_cast<size_t>(OpenGTX::Core::InitApp(env, exports))];
}

__attribute__((constructor)) void RegisterOpenGTXCore() noexcept
{
