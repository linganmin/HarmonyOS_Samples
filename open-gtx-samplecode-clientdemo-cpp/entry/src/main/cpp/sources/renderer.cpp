    ...
    if (clearColor || (clearDepth && fbo.renderBuffer_)) {
        ...
        if (glDiscardFramebufferEXT_) {
            glDiscardFramebufferEXT_(GL_FRAMEBUFFER,
                fbo.renderBuffer_ ?
                    static_cast<GLsizei>(std::size(attachments)) :
                    static_cast<GLsizei>(1), attachments);
...
bool Renderer::InitRender() noexcept
{
    ...
    if (!str) {
        GOLOGE("eglQueryString execution failed");
        return false;
    }
    ...
    auto const printExtensions = [](char const *header, std::string extentions) noexcept {
        GOLOGI("%s extensions:", header);
        std::replace(extentions.begin(), extentions.end(), ' ', '\0');
        std::string_view view(extentions);

        for (;;) {
            GOLOGI("    %s", view.data());

            if (auto const next = view.find('\0'); next != std::string_view::npos) {
    ...
    if (isKHRCreateContext) {
        ...
        EGLint attribs[] =
        {
            ...
            EGL_SAMPLE_BUFFERS, numSampleBuffers,
    ...
    if (!str) {
        GOLOGE("glGetString execution failed");
        return false;
    }
    CheckOpenGLError();
    ...
    auto glDebugMessageCallback = reinterpret_cast<PFNGLDEBUGMESSAGECALLBACKKHRPROC>(
        GetOpenGLExtensionFunction("glDebugMessageCallback")
    );
    ...
    glDebugMessageCallback(&Renderer::GLESDebug, nullptr);
    CheckOpenGLError();

#endif // USE_GL_DEBUG_OUTPUT
    ...
    glRenderbufferStorageMultisampleEXT_ =
        reinterpret_cast<PFNGLRENDERBUFFERSTORAGEMULTISAMPLEEXTPROC>(GetOpenGLExtensionFunction(
            "glRenderbufferStorageMultisampleEXT")
        );

    glDiscardFramebufferEXT_ = reinterpret_cast<PFNGLDISCARDFRAMEBUFFEREXTPROC>(
        GetOpenGLExtensionFunction("glDiscardFramebufferEXT")
    );
...
// NOLINTNEXTLINE - can be made static.
[[maybe_unused]] void Renderer::UpdateTextureUniform(GLuint program, char const *uniformName,
    GLuint index) const noexcept
{
    if(!uniformName){
        GOLOGE("uniformName is nullptr");
        return;
    }
    if(!glIsProgram(program)){
        GOLOGE("glIsProgram execution failed");
        return;
    }
    ...
    GLenum depthFormat, GLenum target, GLuint texture, uint32_t flags) noexcept
{
    ...
    if (nSamples > 1) {
        if (depthRenderBuffer) {
            ...
            glRenderbufferStorageMultisampleEXT_(GL_RENDERBUFFER, static_cast<GLsizei>(nSamples),
                depthFormat, static_cast<GLsizei>(width), static_cast<GLsizei>(height));
        ...
        glFramebufferTexture2DMultisampleEXT_(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, target,
            fbo.texture_, 0, static_cast<GLsizei>(nSamples));
...
void Renderer::AttachShader(uint32_t program, uint32_t shader) noexcept
{
    ...
    if(!glIsShader(shader)){
        GOLOGE("glIsShader execution failed");
        return;
    }
    CheckOpenGLError();
...
uint32_t Renderer::CreateGLSLShader(char const *src, int shaderType) noexcept
{
    ...
    if(!src){
        GOLOGE("CreateGLSLShader execution failed");
        return 0U;
    }
...
void Renderer::GLESDebug(GLenum source, GLenum type, GLuint /*id*/, GLenum severity, GLsizei /*length*/,
    GLchar const *message, void const */*userParam*/)
{
    ...
    if (ignoreMessages_.count(IgnoreMessage(type, severity, source, message)) > 0U) {
        return;
    }

#define DE_TOKEN(x, ref)                                                                                                \
    if ((x) == (ref)) {                                                                                                 \
        return #ref;                                                                                                    \
    }

    auto toSource = [](GLenum src) -> char const* {
        DE_TOKEN(src, GL_DEBUG_SOURCE_API)
        DE_TOKEN(src, GL_DEBUG_SOURCE_WINDOW_SYSTEM)
        DE_TOKEN(src, GL_DEBUG_SOURCE_SHADER_COMPILER)
        DE_TOKEN(src, GL_DEBUG_SOURCE_THIRD_PARTY)
        DE_TOKEN(src, GL_DEBUG_SOURCE_APPLICATION)
        DE_TOKEN(src, GL_DEBUG_SOURCE_OTHER)

        return "Unknown";
    };

    auto toType = [](GLenum t) -> char const* {
        DE_TOKEN(t, GL_DEBUG_TYPE_ERROR)
        DE_TOKEN(t, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR)
        DE_TOKEN(t, GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR)
        DE_TOKEN(t, GL_DEBUG_TYPE_PORTABILITY)
        DE_TOKEN(t, GL_DEBUG_TYPE_PERFORMANCE)
        DE_TOKEN(t, GL_DEBUG_TYPE_MARKER)
        DE_TOKEN(t, GL_DEBUG_TYPE_PUSH_GROUP)
        DE_TOKEN(t, GL_DEBUG_TYPE_POP_GROUP)
        DE_TOKEN(t, GL_DEBUG_TYPE_OTHER)

        return "Unknown";
    };

    auto toSeverity = [](GLenum svrt) -> char const* {
        DE_TOKEN(svrt, GL_DEBUG_SEVERITY_HIGH)
        DE_TOKEN(svrt, GL_DEBUG_SEVERITY_MEDIUM)
        DE_TOKEN(svrt, GL_DEBUG_SEVERITY_LOW)
        DE_TOKEN(svrt, GL_DEBUG_SEVERITY_NOTIFICATION)
...
#undef SPAR_TOKEN
    ...
    constexpr char const format[] = R"__(OpenGL debug output:
>>>
    severity: %s
    type: %s
    source: %s
...
<<<)__";
    ...
    GOLOGE(format, toSeverity(severity), toType(type), toSource(source), message);
    if(assertOnDebugReport_){
        GOLOGE("GLESDebug execution failed");
        return;
    }
}
    ...
    bool linkDefaultAttributes) noexcept
{
    ...
    constexpr uint32_t const attributeIndices[] =
    {
    ...
    GLenum target, GLuint texture, bool immutable) noexcept
{
    ...
    fbo.depthTexture_ = depthFormat == GL_NONE ?
        0U :
        CreateTexture(depthFormat, width, height, 1, target, immutable);
...
void Renderer::OutEGLError(char const *message) noexcept
{
    ...
    if(offset >= errors){
        GOLOGE("OutEGLError execution failed");
        return;
    }
