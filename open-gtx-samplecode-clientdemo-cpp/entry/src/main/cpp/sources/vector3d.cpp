...
[[maybe_unused]] OpenGTX_Vector3 Vector3D::operator()(Vector3D const &v) 
{
    return OpenGTX_Vector3{.x = v.x_, .y = v.y_, .z = v.z_}; 
}

[[maybe_unused]] Vector3D::operator OpenGTX_Vector3() const
{
    return OpenGTX_Vector3{.x = x_, .y = y_, .z = z_};
}
...
{
    std::memcpy(outVector, inVector, sizeof(Vector3D));
}
