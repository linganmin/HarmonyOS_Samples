...
mediump vec3 GetFinalNormalView(in mediump vec3 tangentView, in mediump vec3 bitangentView, in mediump vec3 normalView, in highp vec2 flippedUV)
{
...
void OpaqueLayer::Instance::UpdateTransform(GPUInstance &gpuInstance, Matrix4x4 const &view, Matrix4x4 const &viewProj,
...
void OpaqueLayer::Culler::Execute(Matrix4x4 const &view, Matrix4x4 const &viewProj,
    float deltaTime) noexcept
{
    {
...
bool OpaqueLayer::Init() noexcept
{
    ...
    // clang-format off
    auto cullBody = [](std::reference_wrapper<Culler> culler, AABB const &bounds,
    ...
    constexpr size_t batch = INSTANCE_COUNT / CULLERS;
    size_t offset = 0U;
    ...
    for (size_t i = 0U; i < limit; ++i ) {
        ...
        culler.thread_ = std::thread(cullBody, std::ref(culler), bounds,
            std::span<Instance>(instances_ + offset, batch));
    ...
    culler.thread_ = std::thread(cullBody, std::ref(culler), bounds,
        std::span<Instance>(instances_ + offset, INSTANCE_COUNT - offset));

    return true;
}

void OpaqueLayer::Render(float deltaTime, Matrix4x4 const &view,
    Matrix4x4 const &viewProj) noexcept
{
    for (auto &culler : cullers_) {
